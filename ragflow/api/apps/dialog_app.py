...
def set_dialog():
    ...
    icon = req.get("icon", "")
    top_n = req.get("top_n", 5)  # Optimized from 6 to 5
    top_k = req.get("top_k", 1024)
    rerank_id = req.get("rerank_id", "")
    if not rerank_id:
        req["rerank_id"] = ""
    similarity_threshold = req.get("similarity_threshold", 0.6)  # Optimized from 0.1 to 0.6
    vector_similarity_weight = req.get("vector_similarity_weight", 0.7)  # Optimized from 0.3 to 0.7
